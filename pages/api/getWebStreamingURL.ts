...
//     console.log(`Web Streaming URL 生成成功：${streamingUrl}`);  

//     res.json({ streamingUrl });  
//   } catch (error) {  
//     if (error instanceof Error) {  
//       console.error(`生成 Web 串流 URL 發生錯誤，電子郵件: ${email}`, error);  
//       res.status(500).json({ error: error.message || '生成 Web 串流 URL 時發生錯誤' });  
//     } else {  
//       console.error(`生成 Web 串流 URL 發生未知錯誤，電子郵件: ${email}`, error);  
//       res.status(500).json({ error: '生成 Web 串流 URL 時發生未知錯誤' });  
//     }  
//   }  
// }
